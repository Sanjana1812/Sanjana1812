<div align="center">
  <h1>Hi there 👋, I'm Sanjana Parachuru</h1>
  <p><h3>Full-Stack Developer | Software Engineer | AI Researcher</h3></p>
  <p>
    <a href="https://www.linkedin.com/in/padma-sanjana" target="_blank">
      <img src="https://img.shields.io/badge/LinkedIn-0077B5?style=for-the-badge&logo=linkedin&logoColor=white" alt="LinkedIn" />
    </a>
    <a href="mailto:sanjana0318@gmail.com" target="_blank">
      <img src="https://img.shields.io/badge/Gmail-D14836?style=for-the-badge&logo=gmail&logoColor=white" alt="Gmail" />
    </a>
  </p>
</div>

---

## 🎓 About Me

- 💻 **Full-Stack Developer** at **Tech Birds Consulting**, specialized in React, Python, and Java.
- 🎓 B.Tech in **Computer Science & Engineering** from **Presidency University, Bangalore**.
- 🚀 Passionate about building high-performance e-commerce platforms and AI-driven automation.
- 📚 Published Researcher in **Computer Vision** (ResNet-50) and **Edge AI**.

---

## 🛠️ My Tech Stack

<p align="left">
  <a href="https://skillicons.dev">
    <img src="https://skillicons.dev/icons?i=react,js,ts,python,java,django,fastapi,postgres,supabase,html,css,git,docker,linux,vercel,pyv" />
  </a>
</p>

---

## 🚀 Featured Projects

### 🏗️ Full-Stack & E-Commerce
* **[Ksheerabdhi](https://github.com/padma-sanjana)** | *Flagship Saree E-Commerce*
    * A premium shopping experience built with **React** and **FastAPI**, featuring a high-contrast tech aesthetic.
* **[MangoMe](https://github.com/padma-sanjana)** | *Premium Produce Platform*
    * E-commerce site for premium mangoes using **React, TypeScript, and Supabase**, deployed on Vercel.
* **[Brain Birds AI](https://github.com/padma-sanjana)** | *Intelligence-Led Engineering*
    * The specialized AI wing for Tech Birds Consulting, focusing on dark-mode "Cyber-Purple" UI/UX.

### 📚 Published Research
* **Waste Classification System** | *IEEE (ICWITE 2025)*
    * AI sorting with **97.3% accuracy** using **PyTorch** and ResNet-50.
    * [View Paper](https://ieeexplore.ieee.org/document/11306921) | [Live Demo](https://e-waste-facility-locator-frontend.onrender.com)
* **Driver Alertness Detection** | *IJIRT*
    * Real-time computer vision using **OpenCV** and Raspberry Pi.
    * [View Publication](https://ijirt.org/article?manuscript=171937)

---

## 📊 GitHub Analytics

<div align="center">
  <img src="https://github-readme-stats.vercel.app/api?username=Sanjana1812&show_icons=true&theme=tokyonight&hide_border=true&count_private=true" alt="Sanjana's Stats" />
  <br/>
  <img src="https://github-readme-stats.vercel.app/api/top-langs/?username=Sanjana1812&layout=compact&theme=tokyonight&hide_border=true" alt="Top Languages" />
</div>

<div align="center">
  <img src="https://github-readme-streak-stats.herokuapp.com/?user=Sanjana1812&theme=tokyonight&hide_border=true" alt="GitHub Streak" />
</div>

---

<div align="center">
  <p><i>"Engineering seamless automation and high-quality user experiences."</i></p>
</div>
